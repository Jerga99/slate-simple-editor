import React, { useRef, useCallback, useEffect, useState, useLayoutEffect, createContext, useContext, useMemo, Fragment } from 'react';
...
const BlogEditor = _ref4 => {
  ...
  const editor = useMemo(() => withLayout(withHistory(withReact(createEditor$1()))), []);
...
const SlateView = _ref => {
  ...
  const editor = useMemo(() => withReact(createEditor$1()), []);
  const renderElement = useCallback(props => /*#__PURE__*/React.createElement(Element, props), []);
  const renderLeaf = useCallback(props => /*#__PURE__*/React.createElement(Leaf, props), []);
